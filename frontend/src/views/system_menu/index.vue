                    ...
                    <NSpace justify="center" size={8} wrap={false}>
                        ...
                        {showUpdate.value ? (
                            <NPopconfirm onPositiveClick={() => handleStatusChange(row.id, nextStatus)}>
                                {{
                                    default: () => $t('page.menu.statusConfirm', {action: actionText, name: row.menuName}),
                                    trigger: () => (
                                        <NButton ghost size="small" type={row.status === 1 ? 'warning' : 'success'} loading={actionLoadingId.value === row.id}>
                                            {actionText}
                                        </NButton>
                                    )
                                }}
                        ...
                        {showDelete.value ? (
                            <NPopconfirm onPositiveClick={() => handleDelete(row.id)}>
                                {{
                                    default: () => $t('page.menu.deleteConfirm', {name: row.menuName}),
                                    trigger: () => (
    ...
    <NCard :bordered="false" class="card-wrapper" size="small">
      <NForm :model="searchParams" :label-width="92" label-placement="left">
        <NGrid :cols="24" :x-gap="16">
          <NFormItemGi :label="$t('page.menu.keyword')" span="24 s:12 m:8">
            <NInput v-model:value="searchParams.keyword" :placeholder="$t('common.keywordSearch')" clearable />
          </NFormItemGi>
          <NFormItemGi :label="$t('page.menu.status')" span="24 s:12 m:6">
            <NSelect
