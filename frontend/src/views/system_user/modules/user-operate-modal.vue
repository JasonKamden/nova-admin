...
<template>
  <NModal v-model:show="visible" preset="card" class="w-760px" :mask-closable="false">
    <template #header>
      ...
      <NForm ref="formRef" :model="model" :rules="rules" :disabled="readonly" :label-width="110" label-placement="left">
        <NGrid :cols="24" :x-gap="16">
          ...
          <NFormItemGi v-if="isAdd" :label="$t('page.user.initialPassword')" path="initialPassword" span="12">
            <NInput v-model:value="model.initialPassword" type="password" show-password-on="click" />
          </NFormItemGi>
          <NFormItemGi :label="$t('page.user.gender')" path="gender" :span="isAdd ? 12 : 12">
            <NSelect
          ...
          <NFormItemGi :label="$t('page.user.bio')" path="bio" span="24">
            <NInput v-model:value="model.bio" type="textarea" :rows="3" />
          </NFormItemGi>
        </NGrid>
      </NForm>
    </NSpin>
