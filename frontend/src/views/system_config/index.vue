...
const {columns, columnChecks, data, loading, getData, getDataByPage, mobilePagination} = useNaivePaginatedTable({
    ...
    columns: () => [
        ...
        {key: 'updateTime', title: $t('page.config.updateTime'), minWidth: 180, render: row => dayjs(row.updateTime).format('YYYY-MM-DD HH:mm:ss')},
        {
            key: 'operate',
            ...
            render: row => (
                <NSpace justify="center" size={8}>
                    <NButton ghost size="small" type="info" onClick={() => openModal('detail', row)}>{$t('common.detail')}</NButton>
                    {showUpdate.value ? <NButton ghost size="small" type="primary" onClick={() => openModal('edit', row)}>{$t('common.edit')}</NButton> : null}
                    {showDelete.value ? (
                        <NPopconfirm onPositiveClick={() => handleDelete(row.id)}>
                            {{
                                default: () => $t('page.config.deleteConfirm', {name: row.configName}),
                                trigger: () => <NButton ghost size="small" type="error" loading={deletingId.value === row.id}>{$t('common.delete')}</NButton>
                            }}
                        </NPopconfirm>
                    ) : null}
                </NSpace>
    ...
    <NCard :bordered="false" class="card-wrapper" size="small">
      <NForm :model="searchParams" :label-width="92" label-placement="left">
        <NGrid :cols="24" :x-gap="16">
          <NFormItemGi :label="$t('page.config.keyword')" span="24 s:12 m:8">
            <NInput v-model:value="searchParams.keyword" :placeholder="$t('common.keywordSearch')" clearable />
          </NFormItemGi>
          <NFormItemGi :label="$t('page.config.configType')" span="24 s:12 m:6">
            <NSelect v-model:value="searchParams.configType" :options="configTypeOptions.map(item => ({label: $t(item.label), value: item.value}))" clearable />
          </NFormItemGi>
          <NFormItemGi :label="$t('page.config.status')" span="24 s:12 m:6">
