...
const {columns: typeColumns, data: typeData, loading: typeLoading, getData: getTypes, scrollX: typeScrollX} = useNaiveTable({
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <NGrid :cols="24" :x-gap="16" class="min-h-0 flex-1">
      <NGi span="24 l:9" class="min-h-0">
        <NCard :bordered="false" :title="$t('page.dictionary.typeTitle')" class="card-wrapper h-full" size="small">
          <template #header-extra>
            <NSpace>
              <NInput v-model:value="typeKeyword" :placeholder="$t('common.keywordSearch')" clearable class="w-220px" />
      ...
      <NGi span="24 l:15" class="min-h-0">
        <NCard :bordered="false" class="card-wrapper h-full" size="small">
          ...
          <template #header-extra>
            <TableHeaderOperation v-model:columns="columnChecks" :loading="dataLoading" @refresh="getDataRows">
              <template #default>
                <NSpace>
                  <NInput v-model:value="dataSearchParams.label" :placeholder="$t('page.dictionary.dataLabel')" clearable class="w-160px" />
                  <NInput v-model:value="dataSearchParams.value" :placeholder="$t('page.dictionary.dataValue')" clearable class="w-160px" />
                  <NSelect
                  ...
                  <NButton v-if="showAdd" :disabled="!activeTypeId" ghost size="small" type="primary" @click="openDataModal('add')">
                    {{ $t('common.add') }}
                  </NButton>
                </NSpace>
              </template>
