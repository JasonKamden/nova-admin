...
export const useTabStore = defineStore(SetupStoreId.Tab, () => {
    ...
    async function removeTab(tabId: string) {
        ...
        // reset route cache
        routeStore.resetRouteCache(removedTabRouteKey as any);
    }
    ...
    async function clearTabs(excludes: string[] = []) {
        ...
        for (const routeKey of routeKeysToReset) {
            routeStore.resetRouteCache(routeKey as any);
        }
    }
     ...
     */
    async function replaceTab(key: string, options?: App.Global.RouterPushOptions) {
