...
export function getDefaultHomeTab(router: Router, homeRouteName: string) {
    const homeRoutePath = getRoutePath(homeRouteName as RouteKey) || `/${homeRouteName}`;
    const i18nLabel = homeRouteName === 'home' ? $t('route.home') : homeRouteName;
...
export function findTabByRouteName(name: string, tabs: App.Global.Tab[]) {
    const routePath = getRoutePath(name as RouteKey) || `/${name}`;
