...
function normalizeI18nLabel(i18nKey?: string | null, fallback?: string) {
    if (i18nKey && i18nKey.startsWith('route.')) {
...
export function updateLocaleOfGlobalMenus(menus: App.Global.Menu[]) {
    ...
    menus.forEach(menu => {
        ...
        const newLabel = normalizeI18nLabel(i18nKey, label);
...
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | ElegantConstRoute) {
    ...
    const label = normalizeI18nLabel(i18nKey as string | null, title);
