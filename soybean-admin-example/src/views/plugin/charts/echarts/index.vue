...
const {domRef: pictorialBarRef} = useEcharts(() => getPictorialBarOption(), {
  onRender() {
  }
});
...
const {domRef: scatterRef} = useEcharts(() => getScatterOption(), {
  onRender() {
  }
});
const {domRef: gaugeRef, setOptions: setGaugeOptions} = useEcharts(() => gaugeOptions, {
  onRender() {
  }
});
